...
| A39 | Productos: **chips de categoría con el conteo de productos de cada una** (sustituyen al `<select>` "Todas las categorías"), servidos por `categoryCounts` del listado (`categoryTally` pagina 10 × 1000 filas porque PostgREST corta en 1000; degrada a 0 sin romper el panel). "Sin categoría" y los chips de categoría se limpian entre sí para no dejar el listado vacío; scroll horizontal en móvil y wrap en escritorio | ✅ |
