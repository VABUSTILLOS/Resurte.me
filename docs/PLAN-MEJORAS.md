...
| A44 | Productos: **scroll infinito del catálogo** en `/admin/productos` (el paginador desaparece). La API no cambia —sigue paginando en el servidor (`page`/`pageSize`, máx 1000)— y el panel acumula las tandas: un `IntersectionObserver` sobre un centinela al final del listado pide la siguiente con `rootMargin: 600px 0px`, y el centinela vive **dentro del scrollport de cada vista** (la tarjeta en tabla, que necesita su propio scroll para el `thead` sticky; la ventana en grid), así que el `root` del observer depende de `view`. La regla de reinicio es la pieza delicada y vive en `pagesToRestore` (`@/lib/admin-product-list`, 5 tests): un listado nuevo (filtros, orden o tanda distintos) arranca en la primera tanda, una **recarga** de los mismos datos (edición, borrado, `reloadKey`) vuelve a cargar hasta la profundidad alcanzada para no devolver al admin al principio, y el primer render respeta el deep-link `?page=N` con tope `MAX_RESTORE_PAGES` (10) para que un `?page=999` no dispare cientos de peticiones. Una tanda que llega tarde (filtros cambiados en vuelo) se descarta por `listRequestRef` en vez de mezclarse; `row-meta` (sync WA, última edición, ventas) se mezcla por tanda en lugar de reemplazarse, y la disponibilidad por ciudad se acumula. El observer se re-registra en cada tanda para encadenar cargas cuando el listado no llena el scrollport, llama siempre a la versión vigente de `loadMore` (ref) para no quedarse con filtros viejos, y el botón **Cargar más** + región viva quedan como fallback accesible; el conteo pasa a «Mostrando X de Y» / «Fin del listado» y el selector de tamaño dice «/ tanda» | ✅ |
...
| A17 | **`iconOnlyButton` (113) e `inputNoName` (135)** medidos por AST y deliberadamente fuera del contrato. **Remedido en la Ronda 23 (`a7`), y la fila estaba mal en las dos cifras y en las dos razones.** *Controles*: no eran 113, eran **25**, y los 25 verdaderos positivos; corregidas las tres vías de falso positivo (`aria-label` por expresión, texto pintado por expresión, props por spread), se arreglaron y **`R7` los congela en cero** con 7 pruebas de detector. *Campos*: no eran 135, eran **193** —el corte 79/114 de la fila no es reproducible porque el script de esa ronda no está en el repo; lo que sí lo es es el 193—. **Cerrado:** el detector de `R8` resuelve las tres vías (vía 1, `<label htmlFor>` en el mismo archivo; vía 2, props del componente contenedor; vía 3, spread → **excluido**, porque no se puede decidir sin salir del archivo) y sobre esos 193 trió **43 defectos reales**: **17 campos sin nombre de ninguna clase** y **26 con una `<label>` hermana que no asociaba nada** —el caso que la fila anticipaba—. Los 26 se arreglaron 22 con `htmlFor`+`id` y 4 con `aria-label` donde la etiqueta vive en un `.map()` y el id colisionaría; los 17 con `aria-label`. **Fuera del perímetro HTML**, 34 usos de `<Input>`/`<Select>`/`<TextArea>` de `comercializacion/ui.tsx` tenían el mismo defecto en otra forma (26 con `<FieldLabel>` hermano): corregidos a mano y también en cero, con el hueco **declarado** en la cabecera (el detector no lee la definición de un componente, así que no los congela). `R8` cierra en **0** con 7 pruebas de detector nuevas (45/45 en el archivo), y las dos mutaciones de prueba —quitar un `aria-label`, quitar un `htmlFor`— fallan con el archivo y la línea exactos. **Queda declarado, no arreglado:** `src/app/panel/foodos/**` tiene **37 hallazgos** (33 etiquetas hermanas sin `htmlFor` y 4 campos mudos) y ese directorio está en `AJENOS`, fuera del contrato por diseño —ver `AU13` | ✅ |
...
| DP1 | **La tercera forma de fila ya tiene contrato.** `### Ronda 13` numera seis filas con el ID como **prefijo** de la celda (`R1 foco`, `R2 diálogos`, `R3`…), forma que `ROW_DEF_RE` no reconocía porque exige la barra inmediatamente después de los dígitos. **Cerrado:** `PREFIX_ROW_RE` la reconoce en un regex **propio**, sin aflojar `ROW_DEF_RE` (su contrato es exigir la barra: si se le quita, cualquier `| A1 algo |` de una tabla ajena entra como fila), y se evalúa **después** de `RANGE_ROW_RE` — si fuera antes, `| R1-R4 |` casaría como prefijo (`-` es palabra válida) y el rango dejaría de expandirse. El defecto real que esto cierra no era una cita colgando: era que **el modelo mentía**. Con la forma reconocida, `R1`–`R4` viven en dos secciones (el rango de §5 Recompensas y la tabla de la Ronda 13), así que un puntero desnudo desde una tercera falla pidiendo calificador, que es lo que el contrato promete. Dos pruebas lo fijan: el canario del recuento (≥6 filas `R1`–`R6` reconocidas) y que `R1`–`R4` aparezcan en `AMBIGUOUS`. Las 7 citas en prosa de `L1116`–`L1163` (`El detalle de R1…`, `El detalle de R5…`, `El detalle de R6…`) **no** son punteros de la forma que el contrato vigila —no usan la forma «fila X»—, así que quedan fuera de su alcance por diseño, no por ceguera | ✅ |
...
| CRM3 | **No se puede verificar RLS real en producción desde el repo**, solo declarar. `crm_tasks` tenía RLS encendida y **0 políticas**: el acceso depende de que todo pase por `createServiceClient()`. **Cerrada en la ronda 23** (el perímetro resultó ser de **24 tablas**, no una, y su modelo de acceso quedó declarado con el archivo que lo demuestra en `rls-declared.contract.test.ts`), y **la mitad que faltaba se midió el 22-sep-2026** con el Management API: las 24 dan `relrowsecurity = true` y **0 políticas**, sus 24 entradas apuntan a archivos que existen y usan `createServiceClient()`, y en la dirección contraria **ninguna** tabla con políticas quedó declarada (cero entradas muertas) ni ninguna declarada tiene RLS apagada. El conjunto que calculan las migraciones (24) y el que mide la base (24) coinciden sin diferencia simétrica; los conteos cuadran (110 `CREATE TABLE` en migraciones + `spatial_ref_sys`, que es de PostGIS y ya estaba excluida por nombre). También se comprobó en vivo el punto más frágil, `consume_rate_limit`: `proacl` sin la entrada `=X` de PUBLIC, `anon_exec = false`, `auth_exec = false`. Cero discrepancias. La medición, el comando reproducible y la tabla de evidencia archivo por archivo viven en `docs/OPS.md` §13. Queda un límite **inherente**, no un pendiente: el contrato estático sigue verificando la declaración, no la base —esto se mide a mano y hay que repetirlo si se tocan esas migraciones— | ✅ |
...
| AU12 | **Quedaban columnas sensibles protegidas solo por la capa de fila, no por la de columna.** El barrido de `AU11` encontró **21 columnas** con `GRANT SELECT` de tabla para `anon`/`authenticated` que no debían ser públicas: los identificadores de Stripe de `orders` y `foodos_orders`, `orders.restore_token`, `addresses.guest_token`, los secretos de `foodos_webhooks`/`foodos_pos_connections`/`foodos_whatsapp_connections`, los tokens de `foodos_couriers`/`foodos_wallet_passes`, la economía de la flotilla y `foodos_order_payments.reviewed_by`. **No eran fugas vivas** (sonda anónima: las 21 devolvían `*/0` filas), así que esto era endurecimiento. **Cerrado con `00195_columnas_sensibles.sql`** (13 tablas, `REVOKE SELECT` de tabla + `GRANT` por columna; `anon` no recibe ninguna de las 21, `authenticated` solo las 9 que su lector de sesión necesita —`secret`, `access_token_enc`, `webhook_secret`, `access_token`, `courier_payout`, `payout_*`, el token de wallet y `tokens_used`—, y `service_role` las 21). **No se revocó escritura** en ninguna tabla: `00160` ya acotó el `UPDATE` de `foodos_restaurants` a 18 columnas y el dueño sí edita su menú. El registro vive en `src/lib/sensitive-columns.ts` con 17 pruebas en `sensitive-columns.contract.test.ts`, probado con **3 mutaciones** (columna privada en un select público, `select("*")`, y una privada añadida al `GRANT`) que fallan con archivo y línea. Antes hubo que cambiar 9 `select("*")` de páginas de **cliente** por listas explícitas —`orders`, `addresses`, `foodos_orders`—: sin eso el `REVOKE` convertía un `[]` en un `42501` para un anónimo. Dos hallazgos que el brief no preveía: un lector más de `orders` (`wallet-actions.ts`) y **un bug real** — `getMesasData` hacía `select("*")` sobre `foodos_orders` con el cliente de sesión, así que el revoke habría roto la cuenta de mesa. Verificado: las 21 en `401`, las 16 sondas de superficie pública en `200`, `verify` y `build` en verde. Queda declarado aparte `AU14` | ✅ |
| AU13 | **El panel de FoodOS tiene los mismos defectos de accesibilidad que A17 acaba de cerrar en el resto.** El detector de `R8` los ve y los cuenta, pero `src/app/panel/foodos/**` está en `AJENOS` —la lista de directorios que el contrato de a11y **no** congela, para que dos líneas de trabajo no se bloqueen entre sí—, así que quedaron medidos y sin tocar: **37 hallazgos en 160 campos repartidos en 28 archivos**, de los cuales **33 son etiquetas hermanas sin `htmlFor`** (el defecto que no asocia nada y que un heurístico de texto daría por bueno) y **4 son campos sin nombre alguno** (`clientes/page.tsx`, `combos/page.tsx`, `cupones/page.tsx`, `menu/page.tsx`, `menu/_components/item-branch-overrides.tsx`, `restaurante/page.tsx`, `restaurante/_components/branch-hours-modal.tsx`, `whatsapp/page.tsx`). El arreglo es mecánico y ya está resuelto 26 veces en A17 (`htmlFor`+`id`, o `aria-label` donde el id colisionaría por vivir en un `.map()`); lo que falta es **sacar el directorio de `AJENOS` y volver a correr `R8`**, que es lo que lo congela. No se hizo aquí para no mezclar el cierre de A17 con una superficie en la que otro flujo trabaja | 🔜 |
| AU14 | **`foodos_orders` conserva dos columnas de dinero que `00193` debería haber escondido con las demás.** `application_fee_amount` (la comisión que se queda la plataforma en un *destination charge*) y `connected_account_id` (la cuenta Express a la que se liquida) son **de la misma clase** que `platform_fee_percent` y `stripe_account_id`, que `00193` sí cerró, y quedaron fuera porque el barrido de `AU12` se acotó a las 21 columnas de su lista. **No hay fuga viva**: la política RLS de `foodos_orders` es de dueño (`EXISTS (… r.user_id = auth.uid())`), así que `anon` no ve filas. El riesgo es el mismo de siempre —el día que una política abra esas filas, la columna queda expuesta sin que nadie toque un `GRANT`—, y el arreglo es el patrón ya probado: añadirlas al registro de `src/lib/sensitive-columns.ts` y al `GRANT` por columna de una migración. Merece su propia fila porque `00193` y `00195` ya cubrieron sus perímetros y esta es la costura entre los dos | 🔜 |
