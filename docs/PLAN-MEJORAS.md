...
| AU11 | **El costo de compra era público en `products` y sigue siéndolo en dos tablas de FoodOS.** `products` tenía `GRANT SELECT` **a nivel de tabla** para `anon`/`authenticated` (del `ALTER DEFAULT PRIVILEGES` de Supabase) y su política RLS es `SELECT USING (true)`. RLS filtra **filas, no columnas**, así que `GET /rest/v1/products?select=cost` devolvía el costo de compra de los 475 productos con la llave anónima que viaja en el navegador: el margen entero, producto por producto, sin cuenta. **Cerrado en `00192_products_column_privileges.sql`** (revoca el `GRANT` de tabla y lo rehace columna por columna, lista blanca en `src/lib/product-columns.ts`, contrato en `src/lib/product-columns.contract.test.ts`); `cost`, `admin_note`, `low_stock_threshold`, `deleted_at` y `publish_at`/`unpublish_at` quedan privadas y el catálogo público pide columnas explícitas. **Lo que queda abierto:** el mismo barrido encontró dos tablas de FoodOS con el patrón intacto y **probado con sonda anónima** (`200`): `foodos_menu_items.cost` (el costo de cada platillo del restaurante — la política `SELECT {public} USING (r.status = 'active')` la expone a cualquiera, y es dato de un **tercero**, el restaurante) y `foodos_restaurants.platform_fee_percent` + `stripe_payouts_enabled` (nuestra comisión por restaurante y su estado de onboarding de Stripe). No se arreglaron aquí **a propósito**: el panel del dueño (`src/app/panel/foodos/actions.ts`, `connect-actions.ts`) lee esas columnas con el cliente de **sesión** (`requireFoodosAuth()`), así que un `REVOKE` mecánico como el de `products` **rompe el panel de restaurantes que pagan**. El arreglo correcto es el de `00162`: revocar la columna y mover esas lecturas a `service_role` detrás de la autorización que ya existe, lo que obliga a tocar ~9 `select("*")` (`src/lib/foodos-public.ts` ×2 —la tienda pública—, `src/app/api/whatsapp/webhook/route.ts`, `src/app/panel/foodos/actions.ts` ×4, `mostrador-actions.ts`, `mesas-actions.ts`). Es un workstream de FoodOS, con su propio playbook, y merece su propia pasada | 🔜 |
