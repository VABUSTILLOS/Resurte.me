...
- ✅ **C5 (ligero)**: bitácora de acciones admin — un único libro (`admin_audit_log`, migración `00072`) escrito por `src/lib/audit-log.ts` (`logAdminAction`), incluidos los cambios de estado/pago/repartidor de `/api/orders/[id]/status`, feed "Bitácora de actividad" en el dashboard `/admin`, API `GET /api/admin/audit-log` (solo admins) y pestaña de auditoría en `/admin/bitacoras`, tests unitarios + e2e de guards (`e2e/admin-audit.spec.ts`). El módulo `src/lib/audit.ts` y su espejo en `notifications` se retiraron en la Ronda 20.
