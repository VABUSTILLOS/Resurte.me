// One-time script to seed 124 new products via Supabase JS client
...
const products = [
  ...
  ['Surimi', 'surimi', 'Palitos de surimi para sushi california y ensaladas.', 'https://images.unsplash.com/photo-1559737558-2b6cc7885f5d?w=600&q=80', 'Del Pacífico', 4, true, 'paquete 250 g'],
...
// Prices per category for store 1 (Resurte.me) and store 2 (Carnemart)
const catPrices = {
...
async function main() {
  ...
  console.log('Inserting 124 products...')
  const batchSize = 50
  for (let i = 0; i < products.length; i += batchSize) {
    const batch = products.slice(i, i + batchSize)
    const { data, error } = await supabase.from('products').insert(
      batch.map(([name, slug, description, image_url, brand, category_id, show_in_whatsapp, unit]) => ({
    ...
    if (error) {
      ...
      // Check for duplicate slug errors — those are OK if product already exists
      if (error.code !== '23505') process.exit(1)
    }
    console.log('  Batch', i / batchSize + 1, '/', Math.ceil(products.length / batchSize), data ? data.length + ' products' : 'some may exist')
  }

  // Step 2: Get all newly created product IDs
  const slugs = products.map(p => p[1])
  const { data: newProducts, error: fetchErr } = await supabase.from('products').select('id,category_id').in('slug', slugs)
  if (fetchErr) { console.error('Fetch error:', fetchErr); process.exit(1) }
  console.log('Fetched', newProducts.length, 'products for pricing')

  // Group by category
  const byCat = {}
  for (const p of newProducts) {
    if (!byCat[p.category_id]) byCat[p.category_id] = []
    byCat[p.category_id].push(p.id)
  }

  // Step 3: Insert prices for store 1 and 2
  let priceIdx = {}
  for (const [catId, ids] of Object.entries(byCat)) {
    for (let i = 0; i < ids.length; i++) {
      const stores = [
        { store_id: 1, price: catPrices[catId].r[i] },
        { store_id: 2, price: catPrices[catId].c[i] },
      ]
      for (const s of stores) {
        ...
        if (error) console.error('Price error:', ids[i], s.store_id, error.message)
      }
    }
    console.log('  Priced cat', catId, ':', ids.length, 'products x 2 stores')
  }

  console.log('Done! 124 products seeded with pricing.')
}
