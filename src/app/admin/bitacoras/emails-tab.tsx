...
import { Download, Mail, RefreshCcw, CheckCircle2, XCircle } from "lucide-react"
...
export function EmailsTab() {
  ...
  // El tope y el total real los manda el servidor: la UI no los adivina.
  ...
  function exportCsv() {
    const csv = toCsv(
      ["Fecha", "Destinatario", "Tipo", "Pedido", "Estado", "Error"],
      filtered.map((l) => [
        new Date(l.sent_at).toLocaleString("es-MX"),
        l.email_to,
        TYPE_LABEL[l.email_type] ?? l.email_type,
        l.order_id ?? "",
        l.status === "sent" ? "Enviado" : "Falló",
        l.error ?? "",
      ])
    )
    const stamp = dayKeyOf(DEFAULT_TIMEZONE)
    downloadCsv(`bitacora-emails-${stamp}.csv`, csv)
  }
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-2 gap-3 flex-wrap">
        <p className="text-sm text-gray-500">
          ...
          {failedCount > 0 && (
            <span className="text-red-700 font-medium"> {failedCount} fallidos en la vista.</span>
          )}
        </p>
      ...
      {/* El corte se declara siempre: una lista capada sin aviso se lee como
          una lista completa. */}
      {!loading && (
        <p className="text-xs text-gray-500 mb-6">
          {resumenDeCorte({ shown: logs.length, cap, truncated, total })}
          {truncated && (
            <span className="text-gray-600">
              {" "}· la consulta se corta en {cap} filas; este filtro es del lado del cliente,
              sobre lo que ya se trajo
            </span>
          )}
          {typeFilter !== "all" && (
            <span className="text-gray-600"> · filtrado a {filtered.length} en la vista</span>
          )}
        </p>
      )}
