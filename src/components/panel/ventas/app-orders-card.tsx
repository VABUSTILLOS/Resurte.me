...
/** i18n sin pluralización: se elige la variante por el conteo. */
function plural(key: string, pluralKey: string, count: number): string {
  return t(count === 1 ? key : pluralKey, { count })
}
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-4 mb-6">
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-bold text-gray-900 text-sm">{t("ventas.appOrders")}</h3>
        <Link
          href="/panel/foodos/pedidos"
          className="flex items-center gap-1 text-xs font-semibold text-indigo-600 hover:text-indigo-700"
        >
          {t("ventas.appOrdersLink")}
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-xl bg-emerald-50 border border-emerald-100 p-3">
          <div className="flex items-center gap-1.5 text-xs text-emerald-700 mb-1">
            ...
            {t("ventas.counterToday")}
          </div>
          <p className="text-lg font-extrabold text-emerald-800">${counterRevenue.toFixed(0)}</p>
          <p className="text-[10px] text-emerald-600">
            {plural("ventas.counterSales", "ventas.counterSalesPlural", counterCount)}
          </p>
        </div>
        <div className="rounded-xl bg-indigo-50 border border-indigo-100 p-3">
          ...
          <p className="text-[10px] text-indigo-600">
            {plural("ventas.appOrdersCount", "ventas.appOrdersCountPlural", summary.todayCount)}
            {summary.pendingCount > 0 &&
              ` · ${plural("ventas.appPendingCount", "ventas.appPendingCountPlural", summary.pendingCount)}`}
          </p>
        </div>
      </div>
      <p className="text-[10px] text-gray-400 mt-2">
        {t("ventas.appWeek", {
          ...
          revenue: summary.weekRevenue.toFixed(0),
        })}
      </p>
    </div>
  )
