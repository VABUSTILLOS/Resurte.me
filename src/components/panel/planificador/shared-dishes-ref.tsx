  ...
  return (
    <div className="bg-gradient-to-br from-emerald-50 to-white rounded-2xl border border-emerald-100 p-5 mb-6">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="text-sm font-semibold text-gray-700">{t("planificador.stepsTitle")}</h3>
      </div>
      <ol className="space-y-2 text-xs text-gray-600">
        {[
          <>1. <Link href="/panel/costeo" className="text-emerald-700 font-semibold hover:underline">{t("planificador.stepCostLink")}</Link>{t("planificador.stepCostRest")}</>,
          <>2. {t("planificador.step2")}</>,
      ...
      {recipes.length > 0 && (
        <details className="mt-4 border-t border-emerald-100 pt-3">
          <summary className="text-xs font-semibold text-emerald-700 cursor-pointer hover:text-emerald-800 transition-colors">
            {t("planificador.suggestedRecipes", { collection: collectionName, count: recipes.length })}
          </summary>
