  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-2 sm:gap-3 mb-4 sm:mb-6">
      <div className="bg-white rounded-xl border border-gray-100 p-3 sm:p-4 text-center">
        ...
        <p className="text-lg sm:text-xl font-extrabold text-gray-900">{stats.dishesCount}</p>
        <p className="text-[11px] text-gray-400">Platillos costeados</p>
        {stats.totalCosteo > 0 && (
          <p className="text-[11px] text-blue-600 font-medium mt-1">Costo: ${stats.totalCosteo.toFixed(0)}</p>
        )}
      </div>
      <div className="bg-white rounded-xl border border-gray-100 p-3 sm:p-4 text-center">
        <div className="w-8 h-8 sm:w-9 sm:h-9 bg-blue-50 rounded-lg flex items-center justify-center mx-auto mb-1.5 sm:mb-2">
        ...
        <p className={`text-lg sm:text-xl font-extrabold ${stats.avgFoodCost > panelCfg.foodCostRedAbove ? "text-red-700" : stats.avgFoodCost > panelCfg.foodCostGreenMax ? "text-amber-700" : "text-green-700"}`}>
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-3 sm:p-4 text-center">
        <div className="w-8 h-8 sm:w-9 sm:h-9 bg-red-50 rounded-lg flex items-center justify-center mx-auto mb-1.5 sm:mb-2">
          <DollarSign className="w-4 h-4 text-red-600" />
        </div>
        <p className={`text-lg sm:text-xl font-extrabold ${stats.totalMerma > 1000 ? "text-red-700" : "text-gray-900"}`}>
          ${stats.totalMerma.toFixed(0)}
        </p>
        <p className="text-[11px] text-gray-400">Pérdida por merma</p>
        {stats.mermaCount > 0 && (
          <p className="text-[11px] text-red-500 font-medium mt-1">{stats.mermaCount} registros</p>
        )}
      </div>
      <div className="bg-white rounded-xl border border-gray-100 p-3 sm:p-4 text-center">
        <div className="w-8 h-8 sm:w-9 sm:h-9 bg-emerald-50 rounded-lg flex items-center justify-center mx-auto mb-1.5 sm:mb-2">
          <BarChart3 className="w-4 h-4 text-emerald-600" />
        </div>
        <p className={`text-lg sm:text-xl font-extrabold ${stats.mermaVsGoal > 100 ? "text-red-700" : "text-emerald-700"}`}>
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-3 sm:p-4 text-center">
        <div className="w-8 h-8 sm:w-9 sm:h-9 bg-green-50 rounded-lg flex items-center justify-center mx-auto mb-1.5 sm:mb-2">
          <TrendingUp className="w-4 h-4 text-green-600" />
        </div>
        <div className="flex items-center justify-center gap-2">
          <span className="text-lg sm:text-xl font-extrabold text-green-700">{stats.green}</span>
          {stats.red > 0 && (
            <span className="text-lg sm:text-xl font-extrabold text-red-500">{stats.red}</span>
          )}
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-3 sm:p-4 text-center">
        <div className="w-8 h-8 sm:w-9 sm:h-9 bg-purple-50 rounded-lg flex items-center justify-center mx-auto mb-1.5 sm:mb-2">
          <ShoppingCart className="w-4 h-4 text-purple-600" />
        </div>
        <p className={`text-lg sm:text-xl font-extrabold ${stats.seasonalSavings > 0 ? "text-purple-700" : "text-gray-400"}`}>
        ...
        {stats.seasonalSavings > 0 && (
          <p className="text-[11px] text-purple-500 font-medium mt-1">Lista de compras</p>
        )}
      </div>
      <div className="bg-white rounded-xl border border-gray-100 p-3 sm:p-4 text-center">
        <div className="w-8 h-8 sm:w-9 sm:h-9 bg-amber-50 rounded-lg flex items-center justify-center mx-auto mb-1.5 sm:mb-2">
          <UtensilsCrossed className="w-4 h-4 text-amber-600" />
        </div>
        <p className={`text-lg sm:text-xl font-extrabold ${mesasInfo.occupied > 0 ? "text-amber-700" : "text-gray-400"}`}>
        ...
        {mesasInfo.longCount > 0 && (
          <p className="text-[11px] text-amber-600 font-medium mt-1">{`${mesasInfo.longCount} > 3 h`}</p>
        )}
      </div>
    </div>
  )
